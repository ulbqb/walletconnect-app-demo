...
export function walletConnect(parameters: WalletConnectParameters) {
  ...
  return createConnector<Provider, Properties, StorageItem>((config) => ({
    ...
    onDisplayUri(uri) {
      ...
      if (params.get('is_line') !== null) {
        window.parent.postMessage({ type: 'display_uri', data: uri }, "*");
      } else {
        const miniWalletLink = process.env['NEXT_PUBLIC_MINI_WALLET_TALL']
        window.open(miniWalletLink + '/wc?uri=' + encodeURIComponent(uri));
      }
